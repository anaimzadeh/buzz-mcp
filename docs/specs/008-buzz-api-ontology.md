...
| `Item` / `ActivityItem` | `buzz.get_item`, `buzz.list_items`, `buzz.get_activity`, `buzz.list_activities`, course item resources | `GetItem`, `GetItemList` |
...
| `buzz.list_items` | `GetItemList` | `Item[]` | Implemented with service-side limit; query, itemid, and allversions deferred. |
